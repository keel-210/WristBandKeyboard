    ...
    public class TrackerHandler : MonoBehaviour
    {
        ...
        void Update()
        {
            if (IsOVRConnected)
            {
                ...
                {
                    RightControllerObject.transform.SetPositionAndRotation(controllerPositions.FirstOrDefault());
                    if (controllerPositions.Count > 1)
                    {
                        LeftControllerObject.transform.SetPositionAndRotation(controllerPositions[1]);
                    }
                }
